...
$SourceFile = Join-Path $PSScriptRoot ' thread_pool_test.cpp'
...
$IncludeFile = Join-Path (Split-Path -Parent $PSScriptRoot) 'include' ' thread_pool.hpp'
...
$IncludeFile = Join-Path (Split-Path -Parent $PSScriptRoot) 'include' ' thread_pool_utils.hpp'
If (Test-Path -Path $IncludeFile)
{
...
If (Get-Command 'clang++' -ErrorAction SilentlyContinue)
{
    ...
    Build-ClangGCC 'clang++' 'clang_light' '-D THREAD_POOL_LIGHT_TEST'
}
Else
{
...
If (Get-Command 'g++' -ErrorAction SilentlyContinue)
{
    ...
    Build-ClangGCC 'g++' 'gcc_light' '-Wuseless-cast -D THREAD_POOL_LIGHT_TEST'
}
Else
{
...
Function Build-MSVC([String] $ExeSuffix, [String] $ExtraFlags = '')
{
    ...
    $Env: THREAD_POOL_MSVC_COMMAND = "cl $SourceFile /I../include /std:c++17 /permissive- /O2 /W4 /EHsc /Fe:$FullExe /Fo:$FullObj $ExtraFlags"
    Write-Command $Env: THREAD_POOL_MSVC_COMMAND
    pwsh -Command {
        $CurrentDir = Get-Location
        & 'C:\Program Files\Microsoft Visual Studio\2022\Community\Common7\Tools\Launch-VsDevShell.ps1'
        Set-Location $CurrentDir
        Invoke-Expression $Env: THREAD_POOL_MSVC_COMMAND
        Exit $LASTEXITCODE
    }
    $Env: THREAD_POOL_MSVC_COMMAND = $Null
    If ($LASTEXITCODE)
    {
        Write-Error "Failed to compile, aborting script! (Exit code: $LASTEXITCODE)"
        Exit-Script $LASTEXITCODE
...
If ($IsWindows)
{
    ...
    If (Test-Path $MSVCEnv)
    {
        ...
        Build-MSVC 'msvc'
        Build-MSVC 'msvc_light' '/D THREAD_POOL_LIGHT_TEST'
    }
    Else
    {
