...
private:
    ...
    bool is_tty           = true; // Assume it's a TTY, you can change this based on actual check
    ...
    void progress(int curr, int tot) {
        if (is_tty && (curr % period == 0)) {
            ...
            if (use_ema) {
                ...
                double dtsum = std::accumulate(deq_t.begin(), deq_t.end(), 0.0);
                int dnsum    = static_cast<int>(std::accumulate(deq_n.begin(), deq_n.end(), 0.0));
                avgrate      = dnsum / dtsum;
            }
            ...
            for (int i = 0; i < ifills; i++)
                std::cout << bars[8];
            if (curr != tot)
                std::cout << bars[static_cast<int>(8.0 * (fills - ifills))];
            ...
            std::cout << '[' << std::setw(4) << curr << '/' << std::setw(4) << tot << " | " << std::fixed
                      << std::setprecision(2) << std::setw(6) << avgrate / div << ' ' << unit << " | " << std::fixed
                      << std::setprecision(2) << dt_tot << 's' << '<' << peta << 's' << "] " << label << " ";
            if (use_colors)
                std::cout << " ";
    ...
    void progress(int curr, int tot) {
        if (is_tty && (curr % period == 0)) {
            ...
            if (use_ema) {
                ...
                double dtsum = std::accumulate(deq_t.begin(), deq_t.end(), 0.0);
                int dnsum    = (int) std::accumulate(deq_n.begin(), deq_n.end(), 0.0);
                avgrate      = dnsum / dtsum;
            }
            ...
            if (use_colors) {
                if (color_transition) {
                    ...
                    hsv_to_rgb((float) (0.0 + 0.01 * pct / 3), (float) 0.65, (float) 1.0, r, g, b);
